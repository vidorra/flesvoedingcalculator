...
class BolProductFeed {
  ...
  async downloadFeed() {
    if (!this.hasCredentials()) {
      throw new Error('Bol.com Product Feed credentials not configured. Set BOL_PRODUCT_FEED_USERNAME and BOL_PRODUCT_FEED_PASSWORD environment variables.')
    }

    try {
      console.log('Downloading Bol.com product feed...')
      ...
      if (!response.ok) {
        throw new Error(`Product feed download failed: ${response.status} - ${response.statusText}`)
      }
      ...
      console.error('Product feed download error:', error)
      throw error
    }
  }
...
/**
 * Enhanced search function that uses local product feed when available
 */
export async function searchProductsEnhanced(query, options = {}) {
  try {
    // Try local search first (faster and more comprehensive)
    const localResults = await bolProductFeed.searchLocalProducts(query, options)
    
    if (localResults.length > 0) {
      console.log(`Found ${localResults.length} products in local feed`)
      return localResults.map(product => ({
        id: product.id,
        title: product.title,
        summary: product.subtitle || product.description?.substring(0, 150),
        price: product.price ? { amount: Math.round(product.price * 100) } : null,
    ...
    console.log('No local results found, falling back to API search')
    const { searchBabyProducts } = await import('./bol-api.js')
    return await searchBabyProducts(query)
