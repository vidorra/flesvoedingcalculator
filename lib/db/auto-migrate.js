...
export async function autoMigrate() {
  ...
  try {
    ...
    // Create snippets table if it doesn't exist (must happen before ALTER TABLE)
    await client.query(`
    ...
    // Create page_snippets table if it doesn't exist
    await client.query(`
    ...
    // Ensure pages table exists and has the inherit_default column
    // (Default/inherit feature). Pages are seeded by the deploy migration;
    // this only guarantees the table + new column exist on every DB.
    await client.query(`
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'pages' AND column_name = 'inherit_default'
        ) THEN
          ALTER TABLE pages ADD COLUMN inherit_default boolean NOT NULL DEFAULT true;
          RAISE NOTICE 'Added inherit_default column to pages';
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'pages migration error (safe to ignore): %', SQLERRM;
      END $$;
    `)
