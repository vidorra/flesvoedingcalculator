...
declare global {
  interface Window {
    ...
    recaptchaWarningLogged?: boolean
  }
}
...
export const useRecaptcha = (): UseRecaptchaReturn => {
  ...
  useEffect(() => {
    ...
    nextPublicVars.forEach((key) => {
      ...
      console.log(
        `  - ${key}: ${value ? `${value.substring(0, 10)}...${value.slice(-3)} (length: ${value.length})` : 'NOT SET'}`
      )
    })
    ...
    console.log(
      ...
      process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID ? `...${process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID.slice(-3)} ✅` : '❌ MISSING'
    )
    console.log(
      '  - NEXT_PUBLIC_EMAILJS_TEMPLATE_ID:',
      process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID ? `...${process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID.slice(-3)} ✅` : '❌ MISSING'
    )
