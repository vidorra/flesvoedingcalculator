...
export async function syncPricesForWebsite(website) {
  ...
  for (let i = 0; i < active.length; i++) {
    ...
    // foute prijs. Amazon-prijzen zetten we daarom handmatig; de sync laat ze
    // ongemoeid. (Bol heeft wel een betrouwbaar partner-endpoint.)
    ...
    try {
      ...
      if (priceData && (priceData.price || priceData.originalPrice)) {
        ...
        errors.push(`Geen prijsdata voor "${snippet.name}"${priceData?._debug ? ` [${priceData._debug}]` : ''}`)
        errorCount++
      }
    } catch (error) {
